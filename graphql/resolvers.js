...
const { Authors, Books, Currencies, sequelize } = require("../models");
...
const Query = {
  ...
  getCurrencies: async (root, args) => {
    try {
      const currencies = await sequelize.query(
        `EXEC currency_hnd @action='${args.action}'`
      );
      console.log(args);
      return currencies[0];
    } catch (err) {
...
const Mutation = {
  createAuthor: async (_, { Abbrv, FirstName, LastName, DOB, DOD }) => {
    try {
      (await Authors) &&
        Authors.create({
          Abbrv,
  ...
  updateAuthor: async (
    _,
    { AuthorID, Abbrv, FirstName, LastName, DOB, DOD }
  ) => {
    try {
      (await Authors) &&
        Authors.update(
  ...
  createBook: async (
    _,
    { ISBN, Abbrv, BookTitle, EditionNumber, TotalPages }
  ) => {
    try {
      (await Books) &&
        Books.create({
  ...
  updateBook: async (
    _,
    { BookID, ISBN, Abbrv, BookTitle, EditionNumber, TotalPages }
  ) => {
    try {
      (await Books) &&
        Books.update(
